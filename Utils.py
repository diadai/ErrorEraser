...
def get_unlearn_loader(data_unlearn, data_test, args, label):
    train_dataloaders = []
    test_dataloaders = []
    train_samples = TensorDataset(
        data_unlearn,
    ...
    train_dataloader = DataLoader(train_samples, sampler=RandomSampler(train_samples), batch_size=args.train_unlearn_batch_size,pin_memory=True)
    test_dataloader = DataLoader(test_samples, sampler=SequentialSampler(test_samples), batch_size=args.train_unlearn_batch_size)
    train_dataloaders.append(train_dataloader)
    test_dataloaders.append(test_dataloader)

    return train_dataloaders, test_dataloaders

def get_unlearn_loader2(data_unlearn, args, label):
    train_dataloaders = []
    test_dataloaders = []
    train_samples = TensorDataset(
        data_unlearn,
    ...
    train_dataloader = DataLoader(train_samples, sampler=RandomSampler(train_samples), batch_size=args.train_unlearn_batch_size,
                                  pin_memory=False)
